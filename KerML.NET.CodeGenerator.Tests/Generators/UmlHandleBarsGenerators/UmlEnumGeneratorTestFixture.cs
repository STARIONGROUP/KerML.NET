...
namespace KerML.NET.CodeGenerator.Tests.Generators.UmlHandleBarsGenerators
{
    ...
    public class UmlEnumGeneratorTestFixture
    {
        ...
        private UmlEnumGenerator umlEnumGenerator;
        ...
        public void SetUp()
        {
            ...
            var path = Path.Combine("UML", "_KerML2.AutoGenEnum");

            this.enumerationDirectoryInfo = directoryInfo.CreateSubdirectory(path);

            this.umlEnumGenerator = new UmlEnumGenerator();
        }
        ...
        {
            Assert.That(
                async () => await this.umlEnumGenerator.GenerateAsync(xmiReaderResult,
                    enumerationDirectoryInfo),
                Throws.Nothing);
        }

        [Test]
        public async Task Verify_that_expected_enums_are_generated([Values("FeatureDirectionKind", "VisibilityKind")] string enumName)
        {
            var generatedCode = await this.umlEnumGenerator.GenerateEnumerationAsync(xmiReaderResult,
                enumerationDirectoryInfo,
                enumName);

            var expected = await File.ReadAllTextAsync(Path.Combine(TestContext.CurrentContext.TestDirectory,
                $"Expected/UML/AutoGenEnum/{enumName}.cs"));
